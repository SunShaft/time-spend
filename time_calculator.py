    ...
    for i in final_list:
        ...
        for j in i:
            if j.event_type == "A":
                ...
                B_list.append(j)
            elif j.event_type == "C" or j.event_type == "C1" or j.event_type == "C2" or j.event_type == "C3":
...
def pro_data_writer(time_list, date_list):
    for i in range(len(time_list)):
        date = date_list[i].split("/")
        date = "{:>02}".format(date[0]) + "/" + "{:>02}".format(date[1])
        pro_data_list = [j.csv() for j in event_list_creator(time_list[i])]
        csv_file_creator(f"/Users/sbridge/Program/time_spend/pro_data/2023/\
{date}", "/time.csv", pro_data_list)

def pro_data_reader(start_date,end_date,detail = False):
    '''
    ...
    path = "/Users/sbridge/Program/time_spend/pro_data/" 
    date_list = getEveryDay(start_date,end_date)

    print(date_list)

    for date in date_list:
        with open(path+date+"/time.csv", "r") as f:
